...
if __name__ == '__main__':
    ...
    if code != 200:
        ...
        # server酱
        if SENDKEY != '':
            sc_send("方明轩:登录失败，失败原因：" + msg)

        #
        if WX_APP != '':
            wxapp_notify("方明轩:登录失败，失败原因：" + msg)
    else:
        print("登录成功！")
        ...
        try:
            ...
            if SENDKEY != '':
                sc_send("方明轩:打卡失败！")
            if WX_APP != '':
                wxapp_notify('方明轩:😩可以正常登录但是遇到异常，原因不明，请自行打卡', '小北打卡失败')
            os._exit(0)
        ...
        if status == 200:
            ...
            # server酱
            if SENDKEY != '':
                sc_send("方明轩:🙁抱歉打卡失败了，原因未知，请自行手动打卡，谢谢")

            #
            if WX_APP != '':
                wxapp_notify("方明轩:🙁抱歉打卡失败了，请自行手动打卡，谢谢--->失败原因:"+json.loads(respond)['msg'], '打卡失败')
